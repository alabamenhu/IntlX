# EXCEPTIONS LOCALIZATION FOR SPANISH
#
...
'Exception.something-wrong'           => method { "Algo malo ocurrió en {self.WHAT.gist}" },
'Exception.died-with'                 => method { "Murió con {self.^name}"                   },
'Exception.no-backtrace'              => method { '  (pila de llamadas no disponible)'               },
'Exception.unknown'                   => method { "Unthrown {self.^name} with no message"    },
'X::Comp::Group.sorry'                => method { '¡LO SIENTO!'                                   },
'X::Comp::Group.difficulties'         => method { 'Potential difficulties:'                  },
'X::Comp::Group.other-diff'           => method { 'Other potential difficulties:'            },
'X::SecurityPolicy::Eval.payload'     => method { '¡¡¡EVAL es una función muy peligrosa!!!'     },
'X::AdHoc.payload'                    => method { 'Error no explicado'                        },
'X::StubCode.default'                 => method { 'Código stub ejecutado'                       },
'X::Method::NotFound.of-type'         => method { "de tipo '$.typename'" },
...
'X::Method::NotFound.suggestion'      => method { "¿Quisiste decir? '@.suggestions[0]'?" },
'X::Method::NotFound.suggestions'     => method { "¿Quisiste decir uno de estos? { @.suggestions.map( { "'$_'" } ).join(", ") }" },
...
'X::Method::NotFound.possible'        => method { ($.addendum ?? "Otras causas" !! "Unas causas") ~ " posibles son:" },
